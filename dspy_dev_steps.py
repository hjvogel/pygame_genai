...
# install ollama locally, get Openai or groq AI keys - pip install dspygen and run this code gen for new pygames
import dspy
import pygame
import sys
from dspygen.lm.groq_lm import Groq
from dspygen.utils.dspy_tools import init_dspy, init_ol
#from dspygen.rm.dynamical_signature_util import create_dynamic_signature_class # need to merge that into dspygen TBD
from dynamical_signature_util import create_dynamic_signature_class
...
def extract_code_snippet(full_response):
    ...
    if start > 2 and end > start:
        ...
        start = full_response.find("thon") + 1 # thats a hack...
        if start < 10:
            end = len(full_response)
            if full_response[4:7] == "hon": # that too
